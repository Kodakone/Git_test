...
def multilabel_train_val_split(
    ...
    all_cat_ids = sorted(
        {int(label) for t in targets_by_filename.values() for label in t["labels"]}
    )
...
def build_yolo_class_mapping(
    ...
    all_cat_ids = sorted(
        {int(label) for f in split_fnames for label in targets_by_filename[f]["labels"]}
    )

    catid_to_cls = {
        catid: i for i, catid in enumerate(all_cat_ids)
    }  # ex. 1900: 0, 2483: 1,...
    cls_to_catid = {
        i: catid for catid, i in catid_to_cls.items()
    }  # ex. 0: 1900, 1: 2483,...
    ...
    for catid in all_cat_ids:
        if catid not in categoryid_to_name:
            ...
            cls_to_name.append(
                categoryid_to_name[catid]
            )  # ex. '보령부스파정 5mg', '뮤테란캡슐 100mg',...

    if missing:
        print(
            f"[mapping] WARNING: categoryid_to_name에 없는 catid {len(missing)}개"
            f"(예: {missing[:10]})"
        )
...
def make_aug_scaffold(
    v0_root: Path, v1_root: Path, cls_to_name: list[str], force: bool = False
):
...
def compute_max_class_id(labels_root: Path) -> int:
    max_id = -1
    for p in list((labels_root / "train").glob("*.txt")) + list(
        (labels_root / "val").glob("*.txt")
    ):
